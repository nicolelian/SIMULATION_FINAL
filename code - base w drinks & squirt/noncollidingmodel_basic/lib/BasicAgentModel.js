	...
	switch(state){
		case UNORDERED:
			if (hasArrived){
				...
				statistics[0].count++; // number of customers who have arrived at entrance
				...
				if (emptySeats.length>0){
					...
					statistics[2].cumulativeValue =(statistics[2].cumulativeValue + 1); // count of rejected customers in percentage terms
				}
			}
		...
		case EATING:
			if (hasArrived){
				...
				customer.target.row = 18;
				customer.target.col = 20;
				// compute statistics for EATEN customer
				var timeInClinic = currentTime - customer.timeAdmitted;
				var stats;
				if (customer.type=="A"){
					stats = statistics[0];
				}//else{
